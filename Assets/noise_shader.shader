...
Shader "noise_shader" 
{
	...
	SubShader 
	{
		...
		void vert(inout appdata_full IN) 
		{
			...
			float2 uvCoords = TRANSFORM_TEX(IN.uv, _NoiseTex);
	
			float4 noiseTexSample = tex2D(_NoiseTex, uvCoords);
			float whiteness = length(noiseTexSample);

			//Convert the position back to local
            IN.vertex = UnityObjectToClipPos(IN.vertex);
			IN.vertex.z += whiteness * 10.0;
		}
